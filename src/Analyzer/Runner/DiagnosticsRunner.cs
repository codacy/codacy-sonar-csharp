...
namespace CodacyCSharp.Analyzer.Runner {
	public class DiagnosticsRunner {
		...
		public DiagnosticsRunner (
			...
			{
				this.options = new AnalyzerOptions (ImmutableArray.Create<AdditionalText> (additionalFiles));
			}
		}

		public Task<ImmutableArray<Diagnostic>> GetDiagnostics (Compilation compilation, CancellationToken cancellationToken) {
			...
			var compilationOptions = (CompilationOptions) new CSharpCompilationOptions (OutputKind.DynamicallyLinkedLibrary);
			...
			return compilationWithAnalyzer.GetAnalyzerDiagnosticsAsync ();
		}
	}
}
